...
- **Animated Background System**: Sparse constellation (C+D hybrid) — 5 blue, 3 red, 2 white particles drifting very slowly (~15 fps, 66 ms tick). Each particle has a large soft radial glow halo. Thin arc connections form when two non-white particles come within 200 px. 8 faint vertical energy streaks drift upward for texture. Ambient concentric glow rings: fire-orange upper-left, electric-blue upper-right, mirroring the logo's yin-yang halves. EKG heartbeat polyline scrolls at 74% screen height with zero lag (offset updated inside the main setInterval, no Animated.View). No user customization; one unified look across all screens.
